...
#define SPARK_GLM52_PP13_SERVICE_BACKEND_QUEUE_DEPTH_PER_SPARK 2u
...
_Static_assert(
	SPARK_GLM52_PP13_SERVICE_BACKEND_PENDING_DECODE_CAPACITY ==
		SPARK_GLM52_PP13_SERVICE_BACKEND_QUEUE_DEPTH_PER_SPARK,
	"pending decode capacity must match scheduler queue depth");
_Static_assert(
...
static SparkStatus SparkGlm52Pp13ServiceBackendResidentAwaitSubmitResult(SparkGlm52Pp13ServiceBackendState *state)
{
	SparkGlm52CudaResidentIpcHeader header;
