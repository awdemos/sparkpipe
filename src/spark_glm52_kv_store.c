	...
	uint32_t queue_depth)
{
	if (lookahead_packet_count == 0u)
		lookahead_packet_count = SPARK_GLM52_KV_STORE_DEFAULT_LOOKAHEAD_PACKETS;
	if (lookahead_packet_count > SPARK_GLM52_KV_STORE_MAX_LOOKAHEAD_PACKETS)
		lookahead_packet_count = SPARK_GLM52_KV_STORE_MAX_LOOKAHEAD_PACKETS;
	...
	const uint32_t *cumulative_nonresident_block_counts)
{
	...
	lookahead_packet_count = SparkGlm52KvStoreNormalizeLookaheadPacketCount(
		lookahead_packet_count,queue_depth);
	if (lookahead_packet_count == 0u ||
	...
	const SparkGlm52KvStoreConfiguration *configuration)
{
	if (configuration == 0 ||
		...
		configuration->maximum_inflight_batch_count >
			SPARK_GLM52_KV_STORE_MAX_INFLIGHT_BATCHES ||
	...
	const SparkGlm52KvStoreBatch *batch)
{
	...
	if (batch == 0 || batch->abi_version != SPARK_GLM52_KV_STORE_ABI_VERSION ||
		batch->descriptor_bytes != SPARK_GLM52_KV_STORE_BATCH_BYTES ||
		batch->batch_id == 0u || batch->block_count == 0u ||
		batch->block_count > SPARK_GLM52_KV_STORE_MAX_BATCH_BLOCKS)
		return SPARK_STATUS_INVALID_ARGUMENT;
	for (block_index = 0u; block_index < batch->block_count; ++block_index)
	{
		...
		if ((block->operation != SPARK_GLM52_KV_STORE_OPERATION_GET &&
			 block->operation != SPARK_GLM52_KV_STORE_OPERATION_PUT) ||
			block->key_bytes == 0u ||
			block->key_bytes >= SPARK_GLM52_KV_STORE_MAX_KEY_BYTES ||
	...
	uint32_t required_capability_flags)
{
	if (store_interface == 0 ||
		store_interface->abi_version != SPARK_GLM52_KV_STORE_ABI_VERSION ||
		store_interface->descriptor_bytes != SPARK_GLM52_KV_STORE_INTERFACE_BYTES ||
...
{
	SparkGlm52KvStoreGetInterfaceFunction get_interface;
	const SparkGlm52KvStoreInterface *store_interface;
	...
	*(void **)(&get_interface) = dlsym(
		dynamic_library,SPARK_GLM52_KV_STORE_INTERFACE_SYMBOL);
	if (get_interface == 0)
	{
	...
	status = SparkGlm52KvStoreValidateInterface(
