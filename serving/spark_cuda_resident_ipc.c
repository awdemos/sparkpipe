    ...
    uint32_t payload_bytes)
{
    ...
    header->magic = SPARK_CUDA_RESIDENT_IPC_MAGIC;
    header->abi_version = SPARK_CUDA_RESIDENT_IPC_ABI_VERSION;
    header->descriptor_bytes = SPARK_CUDA_RESIDENT_IPC_HEADER_BYTES;
    header->kind = kind;
    ...
    uint32_t maximum_payload_bytes)
{
    ...
    if (header->magic != SPARK_CUDA_RESIDENT_IPC_MAGIC)
        return SPARK_STATUS_PARSE_ERROR;
    if (header->abi_version != SPARK_CUDA_RESIDENT_IPC_ABI_VERSION ||
        header->descriptor_bytes != SPARK_CUDA_RESIDENT_IPC_HEADER_BYTES)
...
static uint32_t SparkCudaResidentIpcCalculateWorkMessageBytes(
	...
	uint32_t maximum_bytes)
{
    ...
    if (work_packet == 0 ||
        ...
        work_packet->descriptor_bytes >
            SPARK_RING_WORK_CONTROL_PACKET_BYTES)
        return 0u;
...
uint32_t SparkCudaResidentIpcCalculateSubmitWorkBytes(
	const SparkRingWorkControlPacket *work_packet)
{
	return SparkCudaResidentIpcCalculateWorkMessageBytes(
		work_packet,
		SPARK_CUDA_RESIDENT_IPC_SUBMIT_WORK_PREFIX_BYTES,
		SPARK_CUDA_RESIDENT_IPC_SUBMIT_WORK_BYTES);
}
    ...
    uint32_t flags)
{
    ...
    if (message == 0 || work_packet == 0 ||
        (flags & ~SPARK_CUDA_RESIDENT_IPC_SUBMIT_WORK_KNOWN_FLAGS) != 0u)
        return SPARK_STATUS_INVALID_ARGUMENT;
    memset(message,0,sizeof(*message));
    message->flags = flags;
    message->work_packet = *work_packet;
    message_bytes = SparkCudaResidentIpcCalculateSubmitWorkBytes(
        &message->work_packet);
    if (message_bytes == 0u)
        return SPARK_STATUS_INVALID_ARGUMENT;
    message->descriptor_bytes = message_bytes;
    return SparkCudaResidentIpcValidateSubmitWork(message,message_bytes);
}
    ...
    uint32_t payload_bytes)
{
    ...
    if (message == 0)
        return SPARK_STATUS_INVALID_ARGUMENT;
    expected_bytes = SparkCudaResidentIpcCalculateSubmitWorkBytes(
        &message->work_packet);
    ...
    if (payload_bytes != expected_bytes ||
        ...
        return SPARK_STATUS_ABI_MISMATCH;
    if ((message->flags &
            ~SPARK_CUDA_RESIDENT_IPC_SUBMIT_WORK_KNOWN_FLAGS) != 0u)
        return SPARK_STATUS_INVALID_ARGUMENT;
	...
	const SparkRingWorkControlPacket *work_packet)
{
	return SparkCudaResidentIpcCalculateWorkMessageBytes(
		...
		SPARK_CUDA_RESIDENT_IPC_SUBMIT_PREFILL_BYTES);
}
    ...
    uint32_t *payload_bytes_out)
{
    ...
    if (payload_bytes > UINT32_MAX ||
        payload_bytes > SPARK_CUDA_RESIDENT_IPC_MAX_DECODE_PAYLOAD_BYTES)
        return SPARK_STATUS_CAPACITY_EXCEEDED;
    ...
    uint32_t maximum_lane_count)
{
    ...
    if (message == 0 || maximum_lane_count == 0u ||
        maximum_lane_count > SPARK_RING_WORK_CONTROL_MAX_LANE_COUNT)
        return SPARK_STATUS_INVALID_ARGUMENT;
    status = SparkCudaResidentIpcDecodePayloadBytes(
    ...
    if ((message->resident_flags &
            ~SPARK_CUDA_RESIDENT_IPC_SUBMIT_KNOWN_FLAGS) != 0u)
        return SPARK_STATUS_INVALID_ARGUMENT;
