...
// The model's side of the validation seam. Each model family's module
...
#ifndef SPARK_RESIDENT_DECODE_STAGE_MAYBE_UNUSED
#if defined(__GNUC__) || defined(__clang__)
#define SPARK_RESIDENT_DECODE_STAGE_MAYBE_UNUSED __attribute__((unused))
#else
#define SPARK_RESIDENT_DECODE_STAGE_MAYBE_UNUSED
#endif
#endif

#if defined(SPARK_RESIDENT_DECODE_STAGE_REQUIRE_EXTERNAL_CUDA_MODULES)
extern SparkStatus SparkResidentDecodeStageBackendVerifyRequiredCudaModules(
...
typedef struct SparkResidentDecodeStageState SparkResidentDecodeStageState;

typedef struct SparkResidentDecodeStagePendingCompletion
{
    atomic_uint state;
    SparkResidentDecodeStageState *owner;
    SparkResidentDecodeStageBackendCompletion backend_completion;
    uint64_t request_id;
...
} SparkResidentDecodeStagePendingCompletion;

enum
{
    SPARK_RESIDENT_DECODE_STAGE_SLOT_AVAILABLE = 0u,
    SPARK_RESIDENT_DECODE_STAGE_SLOT_RESERVED = 1u,
    SPARK_RESIDENT_DECODE_STAGE_SLOT_SUBMITTED = 2u,
    SPARK_RESIDENT_DECODE_STAGE_SLOT_COMPLETING = 3u
};

struct SparkResidentDecodeStageState
{
    SparkModelDriverCompletionFunction completion_function;
    void *completion_context;
    ...
    void *wake_context;
    const SparkResidentDecodeStageNodeContext *node_context;
    const SparkResidentDecodeStageNodeContext *const *stage_slice_node_contexts;
...
static SPARK_RESIDENT_DECODE_STAGE_MAYBE_UNUSED uint64_t SparkResidentDecodeStageNativeActivationScaleBlockSize(
    ...
    uint32_t weight_format)
{
    ...
    payload_bytes = weight_format ==
            SPARK_RESIDENT_DECODE_STAGE_LINEAR_WEIGHT_FORMAT_FP8_E4M3
        ? element_count
        : SparkResidentDecodeStageDivideRoundUpU64(element_count, 2u);
    scale_block_size =
        SparkResidentDecodeStageNativeActivationScaleBlockSize(weight_format);
    if (scale_block_size == 0u)
    {
        return 0u;
    }
    scale_block_count = SparkResidentDecodeStageDivideRoundUpU64(
        linear_plan->input_dimension,
        scale_block_size);
    scale_bytes =
        (uint64_t)linear_plan->maximum_active_sequence_count * scale_block_count;
    return SparkResidentDecodeStageAlignUpU64(payload_bytes, 256u) +
        scale_bytes;
}
...
static uint32_t SparkResidentDecodeStageMinimumU32(
    uint32_t left,
    uint32_t right)
{
...
static uint32_t SparkResidentDecodeStageMaximumU32(
    uint32_t left,
    uint32_t right)
{
    return left > right ? left : right;
...
static void SparkResidentDecodeStageReleaseSlot(
    SparkResidentDecodeStagePendingCompletion *pending_completion)
{
    ...
    atomic_store_explicit(
        ...
        SPARK_RESIDENT_DECODE_STAGE_SLOT_AVAILABLE,
        memory_order_release);
}

static uint32_t SparkResidentDecodeStageNormalizeCompletionTokens(
    SparkResidentDecodeStagePendingCompletion *pending_completion,
    SparkModelDriverCompletion *completion)
{
    ...
    for (token_index = 0u; token_index < token_count; ++token_index)
    {
        if (pending_completion->backend_completion.token_ids[token_index] ==
            SPARK_RESIDENT_DECODE_STAGE_CANCELLED_TOKEN_ID)
            break;
...
static void SparkResidentDecodeStageTryComplete(
    SparkResidentDecodeStagePendingCompletion *pending_completion)
{
    SparkResidentDecodeStageState *state;
    SparkModelDriverCompletion completion;
    SparkModelDriverCompletionFunction completion_function;
    void *completion_context;
    ...
    expected_state = SPARK_RESIDENT_DECODE_STAGE_SLOT_SUBMITTED;
    if (!atomic_compare_exchange_strong_explicit(
            &pending_completion->state,
            &expected_state,
            SPARK_RESIDENT_DECODE_STAGE_SLOT_COMPLETING,
    ...
    (void)SparkResidentDecodeStageNormalizeCompletionTokens(
        pending_completion,
        &completion);
    if (pending_completion->hidden_output_transport_active != 0u)
    {
        ...
        if (completion.status == SPARK_STATUS_BUSY)
        {
            atomic_store_explicit(
                &pending_completion->state,
                SPARK_RESIDENT_DECODE_STAGE_SLOT_SUBMITTED,
                memory_order_release);
            return;
        }
    }
    ...
    SparkResidentDecodeStageReleaseSlot(pending_completion);
    atomic_fetch_add_explicit(
...
static void SparkResidentDecodeStageComplete(void *completion_context)
{
    SparkResidentDecodeStagePendingCompletion *pending_completion;

    pending_completion =
        (SparkResidentDecodeStagePendingCompletion *)completion_context;
    ...
    void **module_state)
{
    ...
    node_context = (const SparkResidentDecodeStageNodeContext *)
    ...
    else
    {
        slice_node_context =
            (const SparkResidentDecodeStageSliceNodeContext *)
                host_services->node_context;
        status = SparkResidentDecodeStageModelValidateSliceNodeContext(
            slice_node_context,
            &node_context);
        if (status != SPARK_STATUS_OK)
        {
    ...
    if (slice_node_context != 0)
    {
        ...
        for (layer_index = 0u;
             ...
             ++layer_index)
        {
            status = SparkResidentDecodeStageBackendVerifyRequiredCudaModules(
    ...
    state = (SparkResidentDecodeStageState *)calloc(1u, allocation_bytes);
    if (state == 0)
    {
         ...
         ++pipeline_slot_index)
    {
        ...
        pending_completion->backend_completion.function =
            SparkResidentDecodeStageComplete;
...
static uint32_t SparkResidentDecodeStageCountSlotsInState(
    const SparkResidentDecodeStageState *state,
...
static void SparkResidentDecodeStageProgressPendingCompletions(
    SparkResidentDecodeStageState *state)
{
    uint32_t pipeline_slot_index;
    ...
    for (pipeline_slot_index = 0u;
         ...
         ++pipeline_slot_index)
    {
        SparkResidentDecodeStageTryComplete(
            &state->pending_completions[pipeline_slot_index]);
    }
}

static uint32_t SparkResidentDecodeStageFindAvailableSlot(
    const SparkResidentDecodeStageState *state)
{
    uint32_t slot_index;
...
static bool SparkResidentDecodeStageDecodeTokenCountIsSupported(
    const SparkResidentDecodeStageState *state,
    uint32_t token_count)
{
    if (state == 0 ||
...
static bool SparkResidentDecodeStageFrameShapeIsSupported(
    const SparkResidentDecodeStageState *state,
    const SparkModelDriverFrame *frame)
{
    ...
    if (!SparkResidentDecodeStageDecodeTokenCountIsSupported(
            state,
            frame->new_token_count))
    {
        return false;
...
static bool SparkResidentDecodeStageStateRequiresRuntimeKvBlockTable(
    const SparkResidentDecodeStageState *state)
{
    uint32_t layer_index;
    ...
    {
        return SparkResidentDecodeStageNodeContextRequiresRuntimeKvBlockTable(
            state->node_context);
    }
    for (layer_index = 0u; layer_index < state->stage_slice_layer_count; ++layer_index)
    {
        if (SparkResidentDecodeStageNodeContextRequiresRuntimeKvBlockTable(
...
static SparkStatus SparkResidentDecodeStageValidateRuntimeKvBlockTableForNodeContext(
    const SparkResidentDecodeStageNodeContext *node_context,
...
static SparkStatus SparkResidentDecodeStageValidateRuntimeKvBlockTable(
    const SparkResidentDecodeStageState *state,
    const SparkKvBlockTableView *runtime_kv_block_table,
    uint32_t active_sequence_count)
{
    ...
    if (state->stage_slice_layer_count == 0u)
    {
        return SparkResidentDecodeStageValidateRuntimeKvBlockTableForNodeContext(
            state->node_context,
    ...
    {
        status =
            SparkResidentDecodeStageValidateRuntimeKvBlockTableForNodeContext(
                state->stage_slice_node_contexts[layer_index],
                runtime_kv_block_table,
                active_sequence_count);
        if (status != SPARK_STATUS_OK)
...
static SparkStatus SparkResidentDecodeStageValidateDsparkHiddenTapFrameContext(
...
static SparkStatus SparkResidentDecodeStageExtractFrameContext(
    ...
    const SparkResidentDecodeStageFrameContext **frame_context_out)
{
    const SparkResidentDecodeStageFrameContext *frame_context;
    ...
    *frame_context_out = 0;
    ...
    frame_context =
        (const SparkResidentDecodeStageFrameContext *)frame->user_context;
    if (frame_context->abi_version !=
        ...
        (frame_context->flags &
            ~SPARK_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_KNOWN_FLAGS) != 0u ||
        frame_context->reserved != 0u ||
        SparkResidentDecodeStageValidateDsparkHiddenTapFrameContext(
    ...
    if ((frame_context->flags &
            SPARK_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_MTP_TREE_VERIFY) != 0u &&
        ((frame_context->flags &
            SPARK_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_LAYER_MAJOR_SPECULATIVE_VERIFY) == 0u ||
...
static SparkStatus SparkResidentDecodeStageExtractRuntimeKvBlockTable(
    const SparkResidentDecodeStageState *state,
    const SparkModelDriverFrame *frame,
    const SparkResidentDecodeStageFrameContext *frame_context,
    const SparkKvBlockTableView **runtime_kv_block_table_out)
{
    ...
    *runtime_kv_block_table_out = 0;
    runtime_kv_block_table_is_required =
        SparkResidentDecodeStageStateRequiresRuntimeKvBlockTable(state);
    ...
    status = SparkResidentDecodeStageValidateRuntimeKvBlockTable(
        state,
        runtime_kv_block_table,
        (frame_context->flags &
            SPARK_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_FLAG_PREFILL_VIEW) != 0u
...
static bool SparkResidentDecodeStageStateRequiresPersistentHiddenTransport(
    const SparkResidentDecodeStageState *state)
{
    uint32_t layer_index;
    ...
    if (state->stage_slice_layer_count == 0u)
    {
        return SparkResidentDecodeStageExecutionFlagIsSet(
            state->node_context,
            SPARK_RESIDENT_DECODE_STAGE_EXECUTION_REQUIRE_PERSISTENT_HIDDEN_TRANSPORT);
    }
    for (layer_index = 0u; layer_index < state->stage_slice_layer_count;
         ++layer_index)
    {
...
static bool SparkResidentDecodeStageStateNeedsInputHiddenTransport(
    const SparkResidentDecodeStageState *state)
{
    if (state == 0)
    {
...
static bool SparkResidentDecodeStageStateNeedsOutputHiddenTransport(
    const SparkResidentDecodeStageState *state)
{
    const SparkResidentDecodeStageNodeContext *output_node_context;
...
static const SparkResidentDecodeStageNodeContext *
SparkResidentDecodeStageOutputNodeContext(
    const SparkResidentDecodeStageState *state)
{
    if (state->stage_slice_layer_count == 0u)
    {
...
static SparkStatus SparkResidentDecodeStagePrepareHiddenTransportPacket(
    const SparkResidentDecodeStageNodeContext *node_context,
...
static SparkStatus SparkResidentDecodeStagePostInputHiddenTransport(
    const SparkResidentDecodeStageState *state,
    const SparkResidentDecodeStageFrameContext *frame_context,
    uint32_t pipeline_slot_index,
    uint32_t active_sequence_count)
{
    const SparkResidentDecodeStageNodeContext *input_node_context;
    const SparkResidentDecodeStagePipelineSlot *pipeline_slot;
    SparkHiddenTransportPacket packet;
    SparkStatus status;

    if (!SparkResidentDecodeStageStateNeedsInputHiddenTransport(state))
    {
        return SPARK_STATUS_OK;
    }
    if (frame_context == 0 ||
    ...
    status = SparkResidentDecodeStagePrepareHiddenTransportPacket(
        input_node_context,
        pipeline_slot,
...
static SparkStatus SparkResidentDecodeStagePrepareOutputHiddenTransport(
    const SparkResidentDecodeStageState *state,
    const SparkResidentDecodeStageFrameContext *frame_context,
    uint32_t pipeline_slot_index,
    uint32_t active_sequence_count,
    SparkHiddenTransportSession **transport_session_out,
    SparkResidentDecodeStageHiddenTransportSendSessionFunction *transport_send_function_out,
    ...
    uint32_t *transport_active_out)
{
    ...
    *transport_active_out = 0u;
    if (!SparkResidentDecodeStageStateNeedsOutputHiddenTransport(state))
    {
        return SPARK_STATUS_OK;
    }
    if (frame_context == 0 ||
    ...
    output_node_context = SparkResidentDecodeStageOutputNodeContext(state);
    ...
    SparkModelDriverFrame *frame)
{
    ...
    state = (SparkResidentDecodeStageState *)module_state;
    if (state == 0 || frame == 0)
    {
        return SPARK_STATUS_INVALID_ARGUMENT;
    }
    if (!SparkResidentDecodeStageFrameShapeIsSupported(state, frame))
    {
        atomic_fetch_add_explicit(
    ...
    status = SparkResidentDecodeStageExtractFrameContext(
        state,
        frame,
        SparkResidentDecodeStageStateRequiresRuntimeKvBlockTable(state) ||
            SparkResidentDecodeStageStateRequiresPersistentHiddenTransport(state),
        &frame_context);
    if (status != SPARK_STATUS_OK)
    {
        atomic_fetch_add_explicit(
    ...
    status = SparkResidentDecodeStageExtractRuntimeKvBlockTable(
        state,
        frame,
        frame_context,
        &runtime_kv_block_table);
    ...
    else
    {
        pipeline_slot_value =
            frame->scalar[
                SPARK_RESIDENT_DECODE_STAGE_PIPELINE_SLOT_SCALAR_INDEX];
    }
    ...
    expected_state = SPARK_RESIDENT_DECODE_STAGE_SLOT_AVAILABLE;
    if (!atomic_compare_exchange_strong_explicit(
            &pending_completion->state,
            &expected_state,
            SPARK_RESIDENT_DECODE_STAGE_SLOT_RESERVED,
            memory_order_acq_rel,
            memory_order_relaxed))
    {
        atomic_fetch_add_explicit(
    ...
    status = SparkResidentDecodeStagePostInputHiddenTransport(
        state,
        frame_context,
        pipeline_slot_index,
        frame->active_slot_count);
    ...
    status = SparkResidentDecodeStagePrepareOutputHiddenTransport(
        state,
        frame_context,
        pipeline_slot_index,
        frame->active_slot_count,
    ...
    if ((frame->flags & SPARK_MODEL_DRIVER_FRAME_FLAG_PREFILL) != 0u)
    {
        if (state->stage_slice_layer_count != 0u)
        {
            status = SparkResidentDecodeStageBackendSubmitStageSlice(
        ...
        {
            status = SparkResidentDecodeStageBackendSubmit(
                state->node_context,
                pipeline_slot_index,
                frame->active_slot_count,
                runtime_kv_block_table,
    ...
    atomic_fetch_add_explicit(
        ...
        1u,
        memory_order_relaxed);
    SparkResidentDecodeStageProgressPendingCompletions(state);
    return SPARK_STATUS_OK;
}
    ...
    SparkModelDriverAdmissionDecision *decision)
{
    ...
    else if (!SparkResidentDecodeStageDecodeTokenCountIsSupported(
        state,
        request->new_token_count))
    {
        decision->rejection_reason =
    ...
    SparkResidentDecodeStageProgressPendingCompletions(state);
    selected_slot = SparkResidentDecodeStageFindAvailableSlot(state);
    available_slot_count = SparkResidentDecodeStageCountSlotsInState(
        state,
        SPARK_RESIDENT_DECODE_STAGE_SLOT_AVAILABLE);
...
void SparkResidentDecodeStageDestroy(void *module_state)
{
    ...
    SparkResidentDecodeStageBackendQuiesce(state->node_context);
    free(state);
}
