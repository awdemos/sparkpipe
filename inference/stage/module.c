...
static bool SparkResidentDecodeStagePointerIsAligned(
    const void *pointer,
    uintptr_t required_alignment);

static bool SparkResidentDecodeStageExecutionFlagIsSet(
    const SparkResidentDecodeStageNodeContext *node_context,
...
static uint64_t SparkResidentDecodeStageDivideRoundUpU64(
    uint64_t value,
    uint64_t divisor)
{
    if (divisor == 0u)
