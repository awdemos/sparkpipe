 ...
 *
 * Version 1 executes DECODE batches (one token per lane per frame) across
 * all three attention kinds, both router paths, and the full mHC
 * machinery. Prefill frames and MTP execution are refused with distinct
 * statuses until their passes land - refusal, never partial execution.
...
#define SPARK_DSV4_RESIDENT_DECODE_STAGE_FRAME_CONTEXT_ABI_VERSION 1u
