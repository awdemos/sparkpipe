    ...
    const uint16_t *taps)
{
    ...
    if (SparkGlm52DsparkDraftBackendTapOutputPointers(
        backend, 0u, tap_outputs, &lane_stride) != SPARK_STATUS_OK)
        return -1;
    if (lane_stride !=
    ...
    const float *expected_confidence)
{
    ...
    if (actual_target == 0 || actual_final == 0)
        return -1;
    if (cudaMemcpy(actual_target, backend->device_target_hidden_bf16,
            SPARK_GLM52_DSPARK_HIDDEN_DIMENSION * sizeof(uint16_t),
            cudaMemcpyDeviceToHost) != cudaSuccess ||
        cudaMemcpy(actual_final, backend->device_block_final_bf16,
            SPARK_GLM52_DSPARK_BLOCK_SIZE * SPARK_GLM52_DSPARK_HIDDEN_DIMENSION *
    ...
    {
        fprintf(stdout,
            "dspark_token index=%u actual=%u expected=%u confidence=%.6f expected_confidence=%.6f\n",
            token_index, result->token_ids[token_index], expected_tokens[token_index],
            backend->host_confidence_f32[token_index], expected_confidence[token_index]);
        if (result->token_ids[token_index] != expected_tokens[token_index] ||
            fabsf(backend->host_confidence_f32[token_index] -
...
int main(int argc,char **argv)
{
    ...
    if (taps == 0 || expected_target == 0 || expected_final == 0)
        return 3;
    if (SparkGlm52DsparkLoadOracle(argv[2], taps, expected_target,
    ...
    configuration.maximum_lane_count = 1u;
    configuration.maximum_context_token_count = 16u;
    configuration.safetensors_path = argv[1];
    if (SparkGlm52DsparkDraftBackendInitialize(
        &backend, &configuration) != SPARK_STATUS_OK)
        return 5;
    if (SparkGlm52DsparkUploadTaps(&backend, taps) != 0)
        return 6;
    cudaEventCreate(&start_event);
    cudaEventCreate(&stop_event);
    cudaEventRecord(start_event, (cudaStream_t)backend.cuda_stream);
    if (SparkGlm52DsparkDraftBackendStageLane(
        &backend, 0u, 1u, 1u,
        SPARK_GLM52_DSPARK_VALIDATION_ANCHOR_TOKEN_ID, 1u) != SPARK_STATUS_OK)
        return 7;
    ...
    request.active_sequence_index = 0u;
    request.sequence_id = 1u;
    request.sequence_position = 1u;
    request.tap_generation = 1u;
    cudaEventRecord(start_event, (cudaStream_t)backend.cuda_stream);
    if (SparkGlm52DsparkDraftBackendDraft(
        &backend, &request, &result) != SPARK_STATUS_OK)
        return 8;
    cudaEventRecord(stop_event, (cudaStream_t)backend.cuda_stream);
    cudaEventSynchronize(stop_event);
    cudaEventElapsedTime(&draft_ms, start_event, stop_event);
    validation_status = SparkGlm52DsparkValidateOutputs(
        &backend, &result, expected_target, expected_final,
        expected_tokens, expected_confidence);
