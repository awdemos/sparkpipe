...
#define SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_PREFILL_TILE_ROWS 16u
...
static __global__ void SparkGlm52ResidentDecodeStageDsaScoreKernel(
    ...
    for (head_index = 0u; head_index < index_head_count; ++head_index)
    {
             ...
             dimension_index += blockDim.x)
        {
            ...
            query_value = SparkGlm52ResidentDecodeStageBf16ToFloat(
                query_index_heads_bf16[
                    ...
                    (uint64_t)dimension_index]);
            key_value = SparkGlm52ResidentDecodeStageBf16ToFloat(
                key_index_cache_bf16[
                    ((uint64_t)cache_slot_index *
                     (uint64_t)index_head_dimension) +
                    (uint64_t)dimension_index]);
            local_sum += query_value * key_value;
        }
        head_score = SparkGlm52ResidentDecodeStageBlockReduceSum(
            local_sum,
            shared_sum);
        head_weight = SparkGlm52ResidentDecodeStageBf16ToFloat(
            index_head_weights_bf16[
                ((uint64_t)sequence_index * (uint64_t)index_head_count) +
                (uint64_t)head_index]);
        score_sum += fmaxf(head_score * index_softmax_scale, 0.0f) *
            head_weight * rsqrtf((float)index_head_count);
    }
    if (threadIdx.x == 0u)
    {
        dsa_token_scores[
...
static __global__ __launch_bounds__(SPARK_GLM52_RESIDENT_DECODE_STAGE_CUDA_THREADS, 2)
void SparkGlm52ResidentDecodeStageDsaScoreWarpCandidateKernel(
    const uint16_t *__restrict__ query_index_heads_bf16,
    const uint16_t *__restrict__ key_index_cache_bf16,
    const uint16_t *__restrict__ index_head_weights_bf16,
    ...
    float index_softmax_scale)
{
    ...
    candidate_index =
        (candidate_group_index * SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_SCORE_CANDIDATES_PER_BLOCK) +
        warp_index;

    if (sequence_index >= active_sequence_count || candidate_index >= dsa_candidate_count)
    {
        return;
    }
    ...
    {
        float head_partial_sums[SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_INDEX_HEAD_COUNT];
        float inverse_head_count_sqrt;
        uint32_t dimension_index;

        for (head_index = 0u;
             head_index < SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_INDEX_HEAD_COUNT;
             ++head_index)
        {
            head_partial_sums[head_index] = 0.0f;
        }
                ...
                query_value = SparkGlm52ResidentDecodeStageBf16ToFloat(
                    query_index_heads_bf16[
                        (((uint64_t)sequence_index * (uint64_t)index_head_count +
                         ...
                         (uint64_t)index_head_dimension) +
                        (uint64_t)dimension_index]);
                head_partial_sums[head_index] += query_value * key_value;
            }
        }
        ...
        for (head_index = 0u; head_index < index_head_count; ++head_index)
        {
            ...
            head_score = SparkGlm52ResidentDecodeStageWarpReduceSum(
                head_partial_sums[head_index]);
            if (lane_index == 0u)
            {
                float head_weight;
...
static __global__ __launch_bounds__(SPARK_GLM52_RESIDENT_DECODE_STAGE_CUDA_THREADS, 1)
void SparkGlm52ResidentDecodeStageFp8MoeBf16ActivationScalarGroupGemmKernel(
    const uint16_t *__restrict__ activation_bf16,
    const uint32_t *__restrict__ activation_source_rows,
    uint32_t activation_row_stride,
    ...
    uint32_t expert_count)
{
    ...
    uint32_t expert_index;
    uint32_t input_scale_block_count;
    uint32_t output_scale_block_count;
    float local_sum;
    float row_sum;

    output_index = blockIdx.x;
    route_index = blockIdx.y;
    if (route_index >= routed_row_count || output_index >= output_dimension)
        return;
    expert_index = packed_expert_ids[route_index];
    if (expert_index >= expert_count)
        return;
    source_row_index = activation_source_rows != 0
        ? activation_source_rows[route_index]
        : route_index;
    input_scale_block_count =
        (input_dimension +
         SPARK_GLM52_RESIDENT_DECODE_STAGE_FP8_MOE_SCALE_BLOCK_SIZE - 1u) /
        SPARK_GLM52_RESIDENT_DECODE_STAGE_FP8_MOE_SCALE_BLOCK_SIZE;
         ...
         input_index += blockDim.x)
    {
        ...
        weight_index =
            ((((uint64_t)expert_index * (uint64_t)output_dimension) +
              (uint64_t)output_index) * (uint64_t)input_dimension) +
            (uint64_t)input_index;
        weight_scale_index =
            ((((uint64_t)expert_index * (uint64_t)output_scale_block_count) +
              (uint64_t)(output_index /
                SPARK_GLM52_RESIDENT_DECODE_STAGE_FP8_MOE_SCALE_BLOCK_SIZE)) *
             (uint64_t)input_scale_block_count) +
            (uint64_t)(input_index /
        ...
        activation_value = SparkGlm52ResidentDecodeStageBf16ToFloat(
            activation_bf16[
                ((uint64_t)source_row_index * (uint64_t)activation_row_stride) +
                (uint64_t)input_index]);
        weight_value = SparkGlm52ResidentDecodeStageFp8E4m3ToFloat(
    ...
    if (threadIdx.x == 0u)
        output_bf16[
            ((uint64_t)route_index * (uint64_t)output_dimension) +
            (uint64_t)output_index] =
            SparkGlm52ResidentDecodeStageFloatToBf16(row_sum);
}

static SparkStatus SparkGlm52ResidentDecodeStageLaunchFp8MoeBf16ActivationScalarGroupGemm(
    const uint16_t *activation_bf16,
    const uint32_t *activation_source_rows,
    uint32_t activation_row_stride,
    ...
    cudaStream_t cuda_stream)
{
    ...
    if (activation_bf16 == 0 || activation_row_stride < input_dimension ||
        packed_expert_ids == 0 || weight_fp8_e4m3 == 0 ||
        weight_scale_inv_f32 == 0 || output_bf16 == 0 ||
        routed_row_count == 0u || routed_row_count > 64u ||
        output_dimension == 0u || input_dimension == 0u ||
        expert_count == 0u || cuda_stream == 0)
        return SPARK_STATUS_INVALID_ARGUMENT;
    grid = dim3(output_dimension, routed_row_count, 1u);
    SparkGlm52ResidentDecodeStageFp8MoeBf16ActivationScalarGroupGemmKernel<<<
        grid,
        SPARK_GLM52_RESIDENT_DECODE_STAGE_CUDA_THREADS,
        0u,
        cuda_stream>>>(
        activation_bf16,
        activation_source_rows,
    ...
    void *cuda_stream_pointer)
{
    ...
    SparkGlm52ResidentDecodeStageFp8MoeGroupedReferenceWorkspaceView workspace_view;
    uint32_t routed_row_count;
    uint32_t use_accurate_bf16_activation;
    dim3 finalize_grid;
    SparkStatus status;
    ...
    cuda_stream = (cudaStream_t)cuda_stream_pointer;
    use_accurate_bf16_activation =
        getenv("SPARKPIPE_FP8_MOE_ACCURATE_BF16_ACTIVATION") != 0;
    SparkGlm52ResidentDecodeStageMoeRouterTopKFromLogitsKernel<<<
        active_sequence_count,
        SPARK_GLM52_RESIDENT_DECODE_STAGE_CUDA_THREADS,
        0u,
    ...
    if (use_accurate_bf16_activation != 0u)
    {
        status = SparkGlm52ResidentDecodeStageLaunchFp8MoeBf16ActivationScalarGroupGemm(
    ...
    SparkGlm52ResidentDecodeStageSiluMulFp8E4m3QuantizeKernel<<<
        ...
        workspace_view.w1_output_bf16,
        use_accurate_bf16_activation != 0u
            ? (uint16_t *)workspace_view.hidden_fp8_e4m3
            : 0,
    ...
    if (use_accurate_bf16_activation != 0u)
        status = SparkGlm52ResidentDecodeStageLaunchFp8MoeBf16ActivationScalarGroupGemm(
            (const uint16_t *)workspace_view.hidden_fp8_e4m3,
            0,
            fp8_moe_plan->intermediate_dimension,
            workspace_view.packed_route_view.packed_expert_ids,
            fp8_moe_plan->w2_weight_fp8_e4m3,
...
static SparkStatus SparkGlm52ResidentDecodeStageLaunchDsaIndexShareScore(
    const SparkGlm52ResidentDecodeStageNodeContext *node_context,
    const SparkGlm52ResidentDecodeStagePipelineSlot *pipeline_slot,
    SparkGlm52ResidentDecodeStageCudaPipelineSlotState *cuda_slot_state,
    cudaStream_t cuda_stream,
    uint32_t active_sequence_count)
{
    ...
    if (node_context == 0 || pipeline_slot == 0 ||
        pipeline_slot->query_index_heads_bf16 == 0 ||
        pipeline_slot->index_head_weights_bf16 == 0 ||
        pipeline_slot->dsa_token_scores == 0 ||
        node_context->key_index_cache_bf16 == 0 ||
        pipeline_slot->block_table == 0 ||
    ...
    score_work_group_count =
        (uint64_t)active_sequence_count * (uint64_t)candidate_group_count;
    ...
    SparkGlm52ResidentDecodeStageDsaScoreWarpCandidateKernel<<<
    ...
    uint32_t active_sequence_count)
{
        ...
        SPARK_GLM52_RESIDENT_DECODE_STAGE_SPARSE_INDEX_DSA_INDEXSHARE_FULL)
    {
        ...
        status = SparkGlm52ResidentDecodeStageLaunchDsaIndexShareScore(
            node_context,
            pipeline_slot,
            cuda_slot_state,
            cuda_stream,
    ...
    uint32_t cache_token_capacity)
{
    ...
    if (node_context->dsa_prefill_scores_f32 == 0 ||
...
prompt_query_latent_bf16 == 0 ||
    ...
    SparkGlm52ResidentDecodeStageAbsorbedQueryCommitKernel<<<
        ...
        0u,
        cuda_stream>>>(
        (const uint16_t *)node_context->dsa_prefill_low_scratch_bf16,
        prompt_query_latent_bf16,
        row_count);
    status = SparkGlm52ResidentDecodeStageCheckCudaLaunch(
        node_context,
    ...
    for (tile_base = 0u; tile_base < row_count;
         tile_base += SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_PREFILL_TILE_ROWS)
    {
        uint32_t tile_rows;

        tile_rows = row_count - tile_base;
        if (tile_rows > SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_PREFILL_TILE_ROWS)
        {
            tile_rows = SPARK_GLM52_RESIDENT_DECODE_STAGE_DSA_PREFILL_TILE_ROWS;
        }
        if (score_selection_active != 0u)
        {
            ...
            SparkGlm52ResidentDecodeStageDsaScoreWarpCandidateKernel<<<
                ...
                node_context->dsa_prefill_scores_f32,
                tile_rows,
                prefill_candidate_bound,
                candidate_group_count,
                node_context->dsa_index_head_count,
    ...
    void *cuda_stream)
{
            ...
            SPARK_GLM52_RESIDENT_DECODE_STAGE_BULK_PREFILL_CAPABILITY_PAGED_ATTENTION) != 0u)
    {
        ...
        if ((node_context->reserved_execution_flags &
            ...
            node_context->dsa_prefill_scores_f32 != 0)
        {
