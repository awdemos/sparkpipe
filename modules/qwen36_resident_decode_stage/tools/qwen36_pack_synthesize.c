...
#define SPARK_QWEN36_SYNTHESIZE_MAX_TENSORS 1024u
#define SPARK_QWEN36_SYNTHESIZE_CHUNK_BYTES (8u * 1024u * 1024u)

typedef struct SparkQwen36SynthesizeContext
{
	SparkQwen36StagePackEntry entries[SPARK_QWEN36_SYNTHESIZE_MAX_TENSORS];
	uint32_t entry_count;
	uint32_t first_layer_index;
	uint32_t layer_count;
	uint64_t payload_cursor;
	uint64_t seed;
} SparkQwen36SynthesizeContext;
...
static uint64_t SparkQwen36SynthesizeNext(uint64_t *state)
{
...
static uint64_t SparkQwen36SynthesizeTensorSeed(uint64_t seed, uint32_t tensor_kind, uint32_t layer_index)
{
...
static uint64_t SparkQwen36SynthesizeAlign(uint64_t offset)
{
	return((offset + SPARK_QWEN36_STAGEPACK_PAYLOAD_ALIGNMENT - 1u) & ~((uint64_t)SPARK_QWEN36_STAGEPACK_PAYLOAD_ALIGNMENT - 1u));
}

static int32_t SparkQwen36SynthesizeAppend(SparkQwen36SynthesizeContext *context, uint32_t tensor_kind, uint32_t layer_index, uint32_t is_global, uint32_t quantize)
{
	SparkQwen36StagePackTensorShape shape;
	SparkQwen36StagePackEntry *entry;
	uint32_t format;
	...
	entry->scale_group_size = format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 ? 32u : 0u;
	entry->payload_bytes = SparkQwen36StagePackPayloadBytes(format,shape.rows,shape.columns);
	entry->scale_bytes = SparkQwen36StagePackScaleBytes(format,shape.rows,shape.columns);
	entry->payload_offset = SparkQwen36SynthesizeAlign(context->payload_cursor);
	entry->scale_offset = entry->scale_bytes != 0u ? SparkQwen36SynthesizeAlign(entry->payload_offset + entry->payload_bytes) : 0u;
...
static int32_t SparkQwen36SynthesizeAppendEveryLayer(SparkQwen36SynthesizeContext *context, uint32_t layer_index, uint32_t quantize)
{
	if ( SparkQwen36SynthesizeAppend(context,SPARK_QWEN36_STAGEPACK_TENSOR_ATTENTION_NORM,layer_index,0u,quantize) < 0 )
		return(-1);
	if ( SparkQwen36SynthesizeAppend(context,SPARK_QWEN36_STAGEPACK_TENSOR_MLP_NORM,layer_index,0u,quantize) < 0 )
		return(-2);
	if ( SparkQwen36SynthesizeAppend(context,SPARK_QWEN36_STAGEPACK_TENSOR_FFN_GATE,layer_index,0u,quantize) < 0 )
		return(-3);
	if ( SparkQwen36SynthesizeAppend(context,SPARK_QWEN36_STAGEPACK_TENSOR_FFN_UP,layer_index,0u,quantize) < 0 )
		return(-4);
	if ( SparkQwen36SynthesizeAppend(context,SPARK_QWEN36_STAGEPACK_TENSOR_FFN_DOWN,layer_index,0u,quantize) < 0 )
		return(-5);
	return(0);
}
...
static int32_t SparkQwen36SynthesizeBuildDirectory(SparkQwen36SynthesizeContext *context, uint32_t quantize)
{
	...
	{
		status = SparkQwen36SynthesizeAppendEveryLayer(context,layer_index,quantize);
		if ( status == 0 )
			status = SPARK_QWEN36_MODEL_LAYER_IS_GDN(layer_index) != 0u ? SparkQwen36SynthesizeAppendGdnLayer(context,layer_index,quantize) : SparkQwen36SynthesizeAppendAttnLayer(context,layer_index,quantize);
		if ( status < 0 )
			return(-2);
	}
...
static void SparkQwen36SynthesizeFillPayload(const SparkQwen36StagePackEntry *entry, uint8_t *buffer, uint64_t bytes, uint64_t *random_state)
{
	...
	if ( entry->weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 )
	{
		for (index = 0; index < bytes; index++)
			buffer[index] = (uint8_t)SparkQwen36SynthesizeNext(random_state);
		return;
	}
	if ( entry->weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_F32 )
	{
...
static void SparkQwen36SynthesizeFillScale(uint8_t *buffer, uint64_t bytes, uint64_t *random_state)
{
	uint64_t index;
	for (index = 0; index < bytes; index++)
		buffer[index] = (uint8_t)(120u + (SparkQwen36SynthesizeNext(random_state) & 7u));
}

static int32_t SparkQwen36SynthesizeWriteRegion(FILE *file, const SparkQwen36StagePackEntry *entry, uint64_t offset, uint64_t bytes, uint32_t is_scale, uint64_t *random_state, uint8_t *chunk)
{
	uint64_t moved,step;
	...
	{
		step = bytes - moved;
		if ( step > SPARK_QWEN36_SYNTHESIZE_CHUNK_BYTES )
			step = SPARK_QWEN36_SYNTHESIZE_CHUNK_BYTES;
		if ( is_scale != 0u )
			SparkQwen36SynthesizeFillScale(chunk,step,random_state);
		else
			SparkQwen36SynthesizeFillPayload(entry,chunk,step,random_state);
...
static void SparkQwen36SynthesizeShiftPayload(SparkQwen36SynthesizeContext *context, uint64_t payload_base)
{
	uint32_t index;
...
static int32_t SparkQwen36SynthesizeWriteEntries(SparkQwen36SynthesizeContext *context, FILE *file, uint8_t *chunk)
{
	uint64_t random_state;
	uint32_t index;
	for (index = 0; index < context->entry_count; index++)
	{
		const SparkQwen36StagePackEntry *entry = &context->entries[index];
		random_state = SparkQwen36SynthesizeTensorSeed(context->seed,entry->tensor_kind,entry->layer_index);
		if ( SparkQwen36SynthesizeWriteRegion(file,entry,entry->payload_offset,entry->payload_bytes,0u,&random_state,chunk) < 0 )
			return(-1);
		if ( entry->scale_bytes != 0u && SparkQwen36SynthesizeWriteRegion(file,entry,entry->scale_offset,entry->scale_bytes,1u,&random_state,chunk) < 0 )
			return(-2);
	}
	return(0);
}

static int32_t SparkQwen36SynthesizeWrite(SparkQwen36SynthesizeContext *context, const char *path, const SparkQwen36StagePackHeader *header)
{
	FILE *file;
	uint8_t *chunk;
	int32_t status;
	...
	if ( file == 0 )
		return(-1);
	chunk = (uint8_t *)malloc(SPARK_QWEN36_SYNTHESIZE_CHUNK_BYTES);
	if ( chunk == 0 )
	{
		fclose(file);
		return(-2);
	...
	if ( fwrite(header,1,sizeof(*header),file) != sizeof(*header) )
		status = -3;
	if ( status == 0 && fwrite(context->entries,sizeof(SparkQwen36StagePackEntry),context->entry_count,file) != context->entry_count )
		status = -4;
	if ( status == 0 )
		status = SparkQwen36SynthesizeWriteEntries(context,file,chunk);
	free(chunk);
...
static void SparkQwen36SynthesizeReport(const SparkQwen36SynthesizeContext *context, const SparkQwen36StagePackHeader *header)
{
	uint64_t mxfp4_bytes = 0,dense_bytes = 0;
	uint32_t index;
	for (index = 0; index < context->entry_count; index++)
	{
		if ( context->entries[index].weight_format == SPARK_QWEN36_RESIDENT_DECODE_STAGE_WEIGHT_FORMAT_MXFP4_E2M1 )
...
int main(int argc, char **argv)
{
	...
	context.seed = 1u;
	context.layer_count = SPARK_QWEN36_MODEL_LAYER_COUNT;
	...
	if ( context.layer_count == 0u || context.first_layer_index + context.layer_count > SPARK_QWEN36_MODEL_LAYER_COUNT )
	{
		fprintf(stderr,"qwen36_pack_synthesize invalid slice %u+%u of %u\n",context.first_layer_index,context.layer_count,SPARK_QWEN36_MODEL_LAYER_COUNT);
		return(2);
	}
	if ( output_path == 0 && dry_run == 0u )
	{
		fprintf(stderr,"qwen36_pack_synthesize --output is required unless --dry-run\n");
		return(3);
	}
	if ( SparkQwen36SynthesizeBuildDirectory(&context,quantize) < 0 )
	{
		fprintf(stderr,"qwen36_pack_synthesize directory build failed\n");
		return(4);
	}
	payload_base = SparkQwen36SynthesizeAlign(SPARK_QWEN36_STAGEPACK_HEADER_BYTES + ((uint64_t)context.entry_count * SPARK_QWEN36_STAGEPACK_ENTRY_BYTES));
	SparkQwen36SynthesizeShiftPayload(&context,payload_base);
	SparkQwen36StagePackExpectedGeometry(&header,context.first_layer_index,context.layer_count);
	header.directory_offset = SPARK_QWEN36_STAGEPACK_HEADER_BYTES;
	header.file_bytes = payload_base + context.payload_cursor;
	SparkQwen36SynthesizeReport(&context,&header);
	if ( dry_run != 0u )
		return(0);
	if ( SparkQwen36SynthesizeWrite(&context,output_path,&header) < 0 )
	{
		fprintf(stderr,"qwen36_pack_synthesize write failed path=%s\n",output_path);
		return(5);
	}
	fprintf(stderr,"qwen36_pack_synthesize wrote %s\n",output_path);
	return(0);
}
