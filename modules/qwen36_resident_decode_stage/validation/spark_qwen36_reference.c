...
static uint64_t SparkQwen36RefNext(uint64_t *state)
{
...
static float SparkQwen36RefUniform(uint64_t *state)
{
	return(((float)(SparkQwen36RefNext(state) & 0xffffffu) / 8388608.0f) - 1.0f);
}

static void SparkQwen36RefFill(float *buffer, uint64_t count, uint64_t *state)
{
	uint64_t index;
	for (index = 0; index < count; index++)
		buffer[index] = SparkQwen36RefUniform(state) * 0.5f;
}

static float SparkQwen36RefSilu(float value)
{
	return(value / (1.0f + expf(-value)));
}

static float SparkQwen36RefSigmoid(float value)
{
	return(1.0f / (1.0f + expf(-value)));
}

static void SparkQwen36RefL2Norm(const float *input, float *output, uint32_t dimension)
{
	uint32_t element;
	float total = 0.0f;
 ...
 */
static void SparkQwen36RefGdnRecurrence(const float *q, const float *k, const float *v, const float *g, const float *beta, float *state, float *output, uint32_t tokens, uint32_t dk, uint32_t dv)
{
	...
	{
		SparkQwen36RefL2Norm(q + ((uint64_t)token * dk),qn,dk);
		SparkQwen36RefL2Norm(k + ((uint64_t)token * dk),kn,dk);
...
static void SparkQwen36RefConvChannel(const float *input, const float *weight, float *tail, float *output, uint32_t tokens)
{
	float window[4];
	uint32_t token,tap;
	float accumulator;
	for (token = 0; token < tokens; token++)
	{
		...
		output[token] = SparkQwen36RefSilu(accumulator);
	}
...
static void SparkQwen36RefGatedNorm(const float *input, const float *z, const float *weight, float *output, uint32_t dimension, float epsilon)
{
	...
	for (element = 0; element < dimension; element++)
		output[element] = (input[element] * inverse) * weight[element] * SparkQwen36RefSilu(z[element]);
}

// HF rotate-half partial RoPE on the first rope_dim dims of one head vector.
static void SparkQwen36RefRope(float *vector, uint32_t rope_dim, uint32_t position, float theta)
{
 ...
 */
static void SparkQwen36RefAttention(const float *q_fused, const float *k_cache, const float *v_cache, const float *q_norm_weight, float *output, uint32_t group, uint32_t head_dim, uint32_t rope_dim, uint32_t tokens, float epsilon)
{
	...
	for (head = 0; head < group; head++)
	{
		...
		SparkQwen36RefRope(qh,rope_dim,tokens - 1u,10000000.0f);
		maximum = -3.0e38f;
		for (token = 0; token < tokens; token++)
		{
			probability = 0.0f;
		...
		for (element = 0; element < head_dim; element++)
		{
			...
			output[((uint64_t)head * head_dim) + element] = probability * SparkQwen36RefSigmoid(fused[head_dim + element]);
		}
	}
}
...
static int32_t SparkQwen36RefTestCarry(void)
{
	...
	uint32_t index;
	SparkQwen36RefFill(q,128u * 32u,&noise);
	SparkQwen36RefFill(k,128u * 32u,&noise);
	SparkQwen36RefFill(v,128u * 32u,&noise);
	...
	SparkQwen36RefGdnRecurrence(q,k,v,g,beta,state_one,out_one,128u,32u,32u);
	SparkQwen36RefGdnRecurrence(q,k,v,g,beta,state_two,out_two,64u,32u,32u);
	SparkQwen36RefGdnRecurrence(q + (64u * 32u),k + (64u * 32u),v + (64u * 32u),g + 64u,beta + 64u,state_two,out_two + (64u * 32u),64u,32u,32u);
...
static int32_t SparkQwen36RefTestSaturatedDecay(void)
{
	...
	uint32_t index;
	SparkQwen36RefFill(q,32u,&noise);
	SparkQwen36RefFill(k,32u,&noise);
	SparkQwen36RefFill(v,32u,&noise);
	SparkQwen36RefFill(state,32u * 32u,&noise);
	SparkQwen36RefGdnRecurrence(q,k,v,&g,&beta,state,output,1u,32u,32u);
	// With exp(-30) the prior state is annihilated: o = <q_n, k_n> * beta * v
	// with q_n scaled by 1/sqrt(dk).
	SparkQwen36RefL2Norm(q,qn,32u);
	SparkQwen36RefL2Norm(k,kn,32u);
...
static int32_t SparkQwen36RefTestConv(void)
{
	...
	uint32_t index;
	SparkQwen36RefConvChannel(impulse_in,weight,tail_zero,impulse_out,8u);
	for (index = 0; index < 4u; index++)
	{
		delta = fabsf(impulse_out[index] - SparkQwen36RefSilu(weight[3u - index]));
		if ( delta > difference )
			difference = delta;
	}
	SparkQwen36RefFill(input,24u,&noise);
	memset(tail_one,0,sizeof(tail_one));
	memset(tail_two,0,sizeof(tail_two));
	SparkQwen36RefConvChannel(input,weight,tail_one,one_pass,24u);
	SparkQwen36RefConvChannel(input,weight,tail_two,two_pass,10u);
	SparkQwen36RefConvChannel(input + 10u,weight,tail_two,two_pass + 10u,14u);
...
static int32_t SparkQwen36RefTestGatedNorm(void)
{
	...
	uint32_t index;
	SparkQwen36RefFill(input,128u,&noise);
	SparkQwen36RefFill(z,128u,&noise);
	for (index = 0; index < 128u; index++)
		weight[index] = 1.0f + (0.1f * SparkQwen36RefUniform(&noise));
	SparkQwen36RefGatedNorm(input,z,weight,output,128u,1e-6f);
	for (index = 0; index < 128u; index++)
		variance += (input[index] * input[index]);
	inverse = 1.0f / sqrtf((variance / 128.0f) + 1e-6f);
	for (index = 0; index < 128u; index++)
	{
		expected = input[index] * inverse * weight[index] * SparkQwen36RefSilu(z[index]);
		delta = fabsf(output[index] - expected);
		if ( delta > difference )
			difference = delta;
	}
...
static int32_t SparkQwen36RefTestAttention(void)
{
	...
	SparkQwen36RefFill(q_fused,2u * 2u * 256u,&noise);
	SparkQwen36RefFill(k_cache,48u * 256u,&noise);
	...
	SparkQwen36RefAttention(q_fused,k_cache,v_cache,q_norm,output,2u,256u,64u,48u,1e-6f);
	for (index = 0; index < 2u * 256u; index++)
	{
		const float *fused = q_fused + (((index / 256u) * 2u * 256u) + 256u);
		delta = fabsf(output[index] - (0.25f * SparkQwen36RefSigmoid(fused[index % 256u])));
		if ( delta > difference )
			difference = delta;
	}
	// RoPE inner-product invariance at equal positions on the roped span.
	SparkQwen36RefFill(head,256u,&noise);
	...
	SparkQwen36RefRope(rotated_q,64u,17u,10000000.0f);
	SparkQwen36RefRope(rotated_k,64u,17u,10000000.0f);
	dot_after = 0.0f;
	for (index = 0; index < 64u; index++)
...
int main(void)
{
	...
	if ( SparkQwen36RefTestConv() != 0 )
		return(3);
	if ( SparkQwen36RefTestGatedNorm() != 0 )
		return(4);
	if ( SparkQwen36RefTestAttention() != 0 )
		return(5);
