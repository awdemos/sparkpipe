...
static SparkStatus SparkTestWaitReady(SparkStageKvClient *client, SparkQwen36WorkControlKvState *kv, SparkQwen36WorkControlKvBatchState *batch)
{
	uint32_t poll;
	for (poll = 0; poll < 20000u; poll++)
	{
		assert(SparkQwen36WorkControlProgress(client,kv) == SPARK_STATUS_OK);
		if ( batch->state == SPARK_QWEN36_WORK_CONTROL_BATCH_READY )
			return(batch->status);
...
{
	SparkQwen36WorkControlKvPlanConfig config;
	uint32_t blocks_a[3] = {0u,1u,2u},blocks_b[1] = {7u},blocks_c[2] = {3u,4u};
	SparkQwen36WorkControlPendingLane pending[4];
	uint32_t packet_lane_counts[3] = {2u,1u,1u},cumulative[3],equivalents,selected;
	memset(&config,0,sizeof(config));
	memset(pending,0,sizeof(pending));
	equivalents = SparkQwen36WorkControlGdnBlockEquivalents(200u,96u);
	assert(equivalents == 3u);
	assert(SparkQwen36WorkControlGdnBlockEquivalents(0u,96u) == 0u);
	...
	assert(SparkQwen36WorkControlCumulativeNonresident(pending,4u,packet_lane_counts,3u,equivalents,cumulative) == SPARK_STATUS_OK);
	assert(cumulative[0] == 6u && cumulative[1] == 7u && cumulative[2] == 12u);
	// A miscounted queue is refused, never silently truncated.
	assert(SparkQwen36WorkControlCumulativeNonresident(pending,3u,packet_lane_counts,3u,equivalents,cumulative) == SPARK_STATUS_INVALID_ARGUMENT);
	...
	selected = SparkQwen36WorkControlSelectRestorePackets(&config,3u,cumulative);
	assert(selected == 3u);
...
static void SparkTestLaneAtomicBuild(void)
{
	...
	SparkQwen36WorkControlPendingLane pending[2];
	...
	assert(SparkQwen36WorkControlBuildRestoreBatch(&config,pending,2u,packet_lane_counts,1u,block_staging,4u,gdn_staging,2u,blocks,4u,&block_count,&lanes_built) == SPARK_STATUS_OK);
	...
	assert(SparkQwen36WorkControlBuildRestoreBatch(&config,pending,2u,packet_lane_counts,1u,block_staging,4u,gdn_staging,2u,blocks,8u,&block_count,&lanes_built) == SPARK_STATUS_OK);
...
static void SparkTestTierRoundtrip(void)
{
	...
	SparkQwen36WorkControlKvPlanConfig config;
	SparkQwen36WorkControlKvState kv;
	SparkStageKvClient client;
	SparkKvStoreBlock blocks[SPARK_KV_STORE_MAX_BATCH_BLOCKS];
	SparkQwen36WorkControlPendingLane pending[1];
	...
	client.module_tag = "test_qwen36_wc";
	...
	assert(SparkQwen36WorkControlBuildEvictBatch(&config,777u,resident_blocks,2u,1u,evict_block_staging,evict_gdn_staging,blocks,8u,&block_count) == SPARK_STATUS_OK);
	assert(block_count == 3u && blocks[0].operation == SPARK_KV_STORE_OPERATION_PUT);
	assert(SparkQwen36WorkControlSubmit(&client,&kv.evict,SPARK_KV_STORE_OPERATION_PUT,blocks,block_count,SPARK_QWEN36_WORK_CONTROL_RESTORE_PRIORITY_SPECULATIVE) == SPARK_STATUS_OK);
	...
	assert(SparkQwen36WorkControlSubmit(&client,&kv.evict,SPARK_KV_STORE_OPERATION_PUT,blocks,block_count,1u) == SPARK_STATUS_INVALID_ARGUMENT);
	...
	assert(SparkQwen36WorkControlAcknowledge(&kv.evict) == SPARK_STATUS_OK);
	...
	pending[0].gdn_nonresident = 1u;
	assert(SparkQwen36WorkControlBuildRestoreBatch(&config,pending,1u,packet_lane_counts,1u,restore_block_staging,2u,restore_gdn_staging,1u,blocks,8u,&block_count,&lanes_built) == SPARK_STATUS_OK);
	assert(block_count == 3u && lanes_built == 1u);
	assert(SparkQwen36WorkControlSubmit(&client,&kv.restore,SPARK_KV_STORE_OPERATION_GET,blocks,block_count,SPARK_QWEN36_WORK_CONTROL_RESTORE_PRIORITY_IMMEDIATE) == SPARK_STATUS_OK);
	assert(SparkTestWaitReady(&client,&kv,&kv.restore) == SPARK_STATUS_OK);
	assert(SparkQwen36WorkControlAcknowledge(&kv.restore) == SPARK_STATUS_OK);
	...
	assert(SparkQwen36WorkControlBuildRestoreBatch(&config,pending,1u,packet_lane_counts,1u,restore_block_staging,2u,restore_gdn_staging,1u,blocks,8u,&block_count,&lanes_built) == SPARK_STATUS_OK);
	assert(SparkQwen36WorkControlSubmit(&client,&kv.restore,SPARK_KV_STORE_OPERATION_GET,blocks,block_count,0u) == SPARK_STATUS_OK);
	assert(SparkTestWaitReady(&client,&kv,&kv.restore) != SPARK_STATUS_OK);
	assert(SparkQwen36WorkControlAcknowledge(&kv.restore) == SPARK_STATUS_OK);
...
int main()
{
	...
	std::printf("test_qwen36_work_control PASS\n");
	return(0);
}
