...
static int32_t build(uint64_t rows, uint64_t columns, uint64_t groups, uint32_t box_rows, uint32_t box_columns, uint32_t bits, spark_lm_tensor_map_plan_t *plan)
{
	...
	return(spark_lm_tensor_map_plan_build(&request,plan));
}
...
int32_t main(void)
{
	...
	expect(SPARK_LM_TENSOR_MAP_SWIZZLE_CHUNKS == SPARK_LM_GROUP_GEMM_SWIZZLE_CHUNKS_CONTRACT,
		"descriptor swizzle chunk count equals the kernel's");
	expect(SPARK_LM_TENSOR_MAP_SWIZZLE_CHUNKS * SPARK_LM_TENSOR_MAP_CHUNK_BYTES
		== SPARK_LM_TENSOR_MAP_SWIZZLE_BYTES,
		"8 chunks x 16 bytes equals the 128-byte swizzle span");
	printf("\nFP8 activation tile, M rows x K=6144, box 16x128\n");
	status = build(128u,6144u,1u,16u,128u,SPARK_LM_TENSOR_MAP_BITS_FP8,&plan);
	expect(status == SPARK_LM_TENSOR_MAP_OK,"builds");
	...
	expect(build(4096u,6144u,256u,128u,128u,SPARK_LM_TENSOR_MAP_BITS_NVFP4,&plan)
		== SPARK_LM_TENSOR_MAP_ERR_BOX_ALIGN,
		"TILE_K=128 elements is 64 bytes at NVFP4 and is correctly rejected");
	status = build(4096u,6144u,256u,128u,256u,SPARK_LM_TENSOR_MAP_BITS_NVFP4,&plan);
	expect(status == SPARK_LM_TENSOR_MAP_OK,"TILE_K=256 elements builds");
	...
	expect(build(128u,6144u,1u,16u,128u,6u,&plan) == SPARK_LM_TENSOR_MAP_ERR_BITS,
		"6-bit element width rejected");
	expect(build(128u,6145u,1u,16u,128u,SPARK_LM_TENSOR_MAP_BITS_NVFP4,&plan)
		== SPARK_LM_TENSOR_MAP_ERR_ODD_COLUMNS,
		"odd NVFP4 column count has no byte representation");
	expect(build(128u,320u,1u,16u,128u,SPARK_LM_TENSOR_MAP_BITS_FP8,&plan)
		== SPARK_LM_TENSOR_MAP_ERR_ROW_SWIZZLE,
		"row pitch not a multiple of 128 bytes rejected");
	expect(build(8u,6144u,1u,16u,128u,SPARK_LM_TENSOR_MAP_BITS_FP8,&plan)
		== SPARK_LM_TENSOR_MAP_ERR_BOX_EXCEEDS,
