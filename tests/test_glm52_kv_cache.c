...
static void SparkTestKvCacheAllocatesResidentDeviceBlocks(void)
{
    ...
    assert(SparkKvCacheArenaAcquireBlock(&arena, &first_block) ==
        SPARK_STATUS_OK);
    assert(first_block == 0u);
    assert(SparkKvCacheArenaRetainBlock(&arena, first_block) ==
        SPARK_STATUS_OK);
    assert(blocks[first_block].reference_count == 1u);
    assert(SparkKvCacheArenaMarkBlockResident(&arena, first_block) ==
        SPARK_STATUS_OK);
    assert((blocks[first_block].flags &
        SPARK_KV_CACHE_BLOCK_FLAG_RESIDENT) != 0u);
    assert(SparkKvCacheArenaResolveBlock(
        &arena,
        first_block,
    ...
    assert(SparkKvCacheArenaAcquireBlock(&arena, &second_block) ==
        SPARK_STATUS_OK);
...
static void SparkTestKvCacheCanEvictResidentOwnedBlocks(void)
{
    ...
    assert(SparkKvCacheArenaAcquireBlock(
        &arena,
        &physical_block_index) == SPARK_STATUS_OK);
    assert(SparkKvCacheArenaRetainBlock(
        &arena,
        physical_block_index) == SPARK_STATUS_OK);
    assert(SparkKvCacheArenaMarkBlockResident(
        &arena,
        physical_block_index) == SPARK_STATUS_OK);
    ...
    assert(SparkKvCacheArenaBuildPrefetchPlan(
        &arena,
    ...
    assert(SparkKvCacheArenaMarkPrefetchPlanResident(
        &arena,
        &prefetch_plan) == SPARK_STATUS_OK);
    assert((blocks[physical_block_index].flags &
        SPARK_KV_CACHE_BLOCK_FLAG_RESIDENT) != 0u);
...
static void SparkTestKvCacheBuildsHashAddressedPrefetchPlan(void)
{
    ...
    assert(SparkKvCacheArenaBuildPrefetchPlanFromSourceBlocks(
        &arena,
        &source_block,
        1u,
        1u,
...
static void SparkTestKvCacheResidentEvictionRespectsProtectedBlocks(void)
{
    ...
    assert(SparkKvCacheArenaEvictResidentBlocksToLimit(
        &arena,
        1u,
...
static void SparkTestKvCacheRejectsStalePrefetchGeneration(void)
{
    ...
    assert(SparkKvCacheArenaRecycleBlock(
        &arena,
        physical_block_index) == SPARK_STATUS_OK);
...
static void SparkTestKvCacheEvictsUnprotectedResidentBlocksToLimit(void)
{
    ...
    assert(SparkKvCacheArenaAcquireBlock(&arena, &second_block) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaAcquireBlock(&arena, &third_block) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaMarkBlockResident(&arena, first_block) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaMarkBlockResident(&arena, second_block) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaMarkBlockResident(&arena, third_block) ==
        SPARK_STATUS_OK);
    assert(arena.resident_block_count == 3u);
...
static void SparkTestKvCachePrefetchPlanEvictsColdBlocksAsGroup(void)
{
    ...
    SparkTestInitializeKvCacheArenaWithResidentCapacity(
        ...
        3u,
        2u);
    assert(SparkKvCacheArenaAcquireBlock(&arena, &cold_block_index) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaAcquireBlock(
        &arena,
...
static void SparkTestKvCacheAsyncPrefetchBackendCopiesHashAddressedBlocks(void)
{
    ...
    SparkKvCachePrefetchSourceBlock source_blocks[2u];
    SparkKvCachePrefetchPlan prefetch_plan;
    SparkKvCachePrefetchBackendSourceEntry source_entries[2u];
    ...
    for (block_index = 0u; block_index < 2u; ++block_index)
    {
        ...
        source_blocks[block_index].abi_version = SPARK_KV_CACHE_ABI_VERSION;
        ...
        source_blocks[block_index].flags =
            SPARK_KV_CACHE_PREFETCH_BLOCK_DEFAULT_FLAGS;
    ...
    assert(SparkKvCacheAsyncPrefetchBackendStart(
        &backend,
        99u,
        &prefetch_plan) == SPARK_STATUS_OK);
    assert(SparkKvCacheAsyncPrefetchBackendPoll(
        &backend,
        99u,
...
static void SparkTestKvCacheRejectsRecyclingRetainedBlocks(void)
{
    ...
    SparkTestInitializeKvCacheArena(&arena, blocks, 2u);
    assert(SparkKvCacheArenaAcquireBlock(&arena, &block_index) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaRetainBlock(&arena, block_index) ==
        SPARK_STATUS_OK);
    assert(SparkKvCacheArenaRecycleBlock(&arena, block_index) ==
        SPARK_STATUS_BUSY);
    assert(SparkKvCacheArenaReleaseBlockReference(&arena, block_index) ==
        SPARK_STATUS_OK);
    generation = blocks[block_index].generation;
    assert(SparkKvCacheArenaRecycleBlock(&arena, block_index) ==
        SPARK_STATUS_OK);
    assert(blocks[block_index].generation == generation + 1u);
    assert(SparkKvCacheArenaFreeBlock(&arena, block_index) ==
        SPARK_STATUS_OK);
...
static void SparkTestKvCacheCapacityEstimatorAccountsForMlaCompression(void)
{
    ...
    SparkKvCacheCapacityEstimate full_estimate;
    SparkKvCacheCapacityEstimate full_fp8_estimate;
    SparkKvCacheCapacityEstimate mla_estimate;
    ...
    assert(SparkKvCacheEstimateCapacity(
        &request,
        &full_estimate) == SPARK_STATUS_OK);

    request.layout = SPARK_KV_CACHE_LAYOUT_FULL_KEY_VALUE_FP8_E4M3;
    request.fp8_scale_block_size = SPARK_GLM52_MODEL_FP8_SCALE_BLOCK;
    assert(SparkKvCacheEstimateCapacity(
        &request,
        &full_fp8_estimate) == SPARK_STATUS_OK);

    request.layout = SPARK_KV_CACHE_LAYOUT_MLA_COMPRESSED;
