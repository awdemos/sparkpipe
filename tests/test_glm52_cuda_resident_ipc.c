    ...
    uint32_t *payload_bytes_out)
{
    ...
    assert(SparkCudaResidentIpcDecodePayloadBytes(
        block_index_count,&payload_bytes) == SPARK_STATUS_OK);
    message = (SparkCudaResidentIpcSubmitDecode *)calloc(1u,payload_bytes);
    assert(message != 0);
    ...
    message->execution_batch_bucket =
        SparkStagePlanSelectBatchBucketValue(lane_count);
    ...
    {
        SparkCudaResidentIpcDecodeLane *lane;
...
static void SparkTestWideDecodePayload(void)
{
    ...
    message = SparkTestBuildWideDecode(
        SPARK_RING_WORK_CONTROL_MAX_LANE_COUNT,17u,&payload_bytes);
    assert(SparkCudaResidentIpcValidateSubmitDecode(
        message,payload_bytes,
        SPARK_RING_WORK_CONTROL_MAX_LANE_COUNT) == SPARK_STATUS_OK);
    message->execution_batch_bucket = SPARK_STAGE_PLAN_BUCKET_B512;
    assert(SparkCudaResidentIpcValidateSubmitDecode(
        message,payload_bytes,
        SPARK_RING_WORK_CONTROL_MAX_LANE_COUNT) ==
        SPARK_STATUS_INVALID_ARGUMENT);
    message->execution_batch_bucket = SPARK_STAGE_PLAN_BUCKET_B1024;
...
static void SparkTestInternalDirectoryDecodeHasNoBlockPayload(void)
{
    ...
    assert(payload_bytes ==
        SPARK_CUDA_RESIDENT_IPC_SUBMIT_DECODE_HEADER_BYTES);
    message = (SparkCudaResidentIpcSubmitDecode *)calloc(
        1u,payload_bytes);
    assert(message != 0);
    message->descriptor_bytes =
        SPARK_CUDA_RESIDENT_IPC_SUBMIT_DECODE_HEADER_BYTES;
    ...
    message->dispatch_kind =
        SPARK_REQUEST_API_DISPATCH_KIND_DECODE_BATCH;
    message->lane_count = SPARK_RING_WORK_CONTROL_MAX_LANE_COUNT;
...
static void SparkTestWideSubmitWorkUsesVariablePayload(void)
{
    ...
    packet = (SparkRingWorkControlPacket *)calloc(1u,sizeof(*packet));
    assert(packet != 0);
    message = (SparkCudaResidentIpcSubmitWork *)calloc(
        1u,sizeof(*message));
    assert(message != 0);
    ...
    packet_bytes = SparkRingWorkControlCalculatePacketBytes(
        packet->lane_count);
    assert(packet_bytes == SPARK_RING_WORK_CONTROL_PACKET_BYTES);
    ...
    assert(submit_bytes == SPARK_CUDA_RESIDENT_IPC_SUBMIT_WORK_BYTES);
    assert(SparkCudaResidentIpcInitializeSubmitWork(
        message,packet,
    ...
    packet->descriptor_bytes =
        SPARK_RING_WORK_CONTROL_PACKET_PREFIX_BYTES - 1u;
    assert(SparkCudaResidentIpcCalculateSubmitWorkBytes(packet) == 0u);
    packet->descriptor_bytes =
        SPARK_RING_WORK_CONTROL_PACKET_BYTES + 1u;
    assert(SparkCudaResidentIpcCalculateSubmitWorkBytes(packet) == 0u);
    packet->descriptor_bytes = packet_bytes;
...
static void SparkTestResidentIpcReaderPreservesFragments(void)
{
    ...
    SparkCudaResidentIpcInitializeHeader(
        &header,SPARK_CUDA_RESIDENT_IPC_KIND_QUERY,3u,91u,
        (uint32_t)sizeof(payload));
    SparkCudaResidentIpcReaderReset(&reader);
    assert(write(sockets[0u],&header,
        SPARK_TEST_RESIDENT_IPC_HEADER_FRAGMENT_BYTES) ==
        SPARK_TEST_RESIDENT_IPC_HEADER_FRAGMENT_BYTES);
    assert(SparkCudaResidentIpcReadHeader(
    ...
    assert(reader.header.kind == SPARK_CUDA_RESIDENT_IPC_KIND_QUERY);
    assert(write(sockets[0u],payload,
        SPARK_TEST_RESIDENT_IPC_PAYLOAD_FRAGMENT_BYTES) ==
        SPARK_TEST_RESIDENT_IPC_PAYLOAD_FRAGMENT_BYTES);
    assert(SparkCudaResidentIpcReadPayload(
...
static void SparkTestPrefillRowsWidenExecutionBucket(void)
{
    ...
    assert(SparkRingWorkControlSelectExecutionBatchBucket(
        ...
        17u,
        &batch_bucket) == SPARK_STATUS_OK);
    assert(batch_bucket == SPARK_STAGE_PLAN_BUCKET_B32);
    assert(SparkRingWorkControlSelectExecutionBatchBucket(
        &dispatch,
        16u,
        &batch_bucket) == SPARK_STATUS_OK);
    assert(batch_bucket == SPARK_STAGE_PLAN_BUCKET_B16);
