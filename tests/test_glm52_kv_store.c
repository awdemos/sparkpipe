	...
	const SparkGlm52KvStoreBatch *batch)
{
	if (store_state == 0)
		return SPARK_STATUS_INVALID_ARGUMENT;
	return SparkGlm52KvStoreValidateBatch(batch);
}
...
static void SparkTestKvStoreLookahead(void)
{
	...
	assert(SparkGlm52KvStoreNormalizeLookaheadPacketCount(0u,8u) == 3u);
	assert(SparkGlm52KvStoreNormalizeLookaheadPacketCount(0u,2u) == 2u);
	assert(SparkGlm52KvStoreNormalizeLookaheadPacketCount(7u,4u) == 4u);
	assert(SparkGlm52KvStoreNormalizeLookaheadPacketCount(99u,99u) == 8u);
	assert(SparkGlm52KvStoreNormalizeLookaheadPacketCount(3u,0u) == 0u);
	assert(SparkGlm52KvStoreSelectPressureLimitedLookaheadPacketCount(
...
static void SparkTestKvStoreContracts(void)
{
	...
	assert(SparkGlm52KvStoreValidateConfiguration(&configuration) ==
		SPARK_STATUS_OK);
	...
	store_interface.capability_flags = SPARK_GLM52_KV_STORE_REQUIRED_CAPS;
	...
	assert(SparkGlm52KvStoreValidateBatch(&batch) == SPARK_STATUS_OK);
	batch.blocks[0u].key[7u] = 'x';
	assert(SparkGlm52KvStoreValidateBatch(&batch) ==
