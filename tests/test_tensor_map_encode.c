...
int main(void){
  ...
  CUtensorMap map; LmTensorMapRequest rq; int32_t st;
  ...
  rq.box_rows=128; rq.box_columns=256; rq.element_bits=LM_TM_BITS_NVFP4;
  st=LmTensorMapPrepare(&map,&rq);
  ck(st==LM_TM_ENCODE_OK,"prepare succeeds");
  ...
  rq.box_rows=16; rq.box_columns=128; rq.element_bits=LM_TM_BITS_FP8;
  st=LmTensorMapPrepare(&map,&rq);
  ck(st==LM_TM_ENCODE_OK,"prepare succeeds");
  ...
  memset(&rq,0,sizeof rq);
  rq.global_address=buf; rq.rows=4096; rq.columns=6144; rq.groups=256;
  rq.box_rows=128; rq.box_columns=128; rq.element_bits=LM_TM_BITS_NVFP4;
  ck(LmTensorMapPrepare(&map,&rq)==LM_TM_ERR_BOX_ALIGN,
