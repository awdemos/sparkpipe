    ...
    const SparkKvCacheConfiguration *configuration)
{
    ...
    {
        SparkKvCacheInitializeBlock(arena, physical_block_index);
    }
    return SPARK_STATUS_OK;
}
    ...
    uint32_t *physical_block_index_out)
{
    ...
    *physical_block_index_out = SPARK_KV_CACHE_NO_BLOCK;
    return SPARK_STATUS_CAPACITY_EXCEEDED;
}
...
static void SparkKvCachePrefetchSourceInitializeFromPhysicalBlock(
    ...
    SparkKvCachePrefetchPlan *prefetch_plan)
{
    ...
    if (physical_block_count > SPARK_KV_CACHE_PREFETCH_BLOCK_CAPACITY)
    {
        return SPARK_STATUS_CAPACITY_EXCEEDED;
    }
    ...
    {
        SparkKvCachePrefetchSourceInitializeFromPhysicalBlock(
...
static uint32_t SparkKvCachePrefetchPlanContainsBlockBeforeIndex(
    const SparkKvCachePrefetchPlan *prefetch_plan,
...
static uint32_t SparkKvCachePhysicalBlockIsProtected(
    const uint32_t *protected_physical_block_indices,
    uint32_t protected_physical_block_count,
    uint32_t physical_block_index)
{
    ...
    uint32_t protected_physical_block_count)
{
    ...
    victim_physical_block_index = SPARK_KV_CACHE_NO_BLOCK;
    victim_block = 0;
    for (physical_block_index = 0u;
         physical_block_index < arena->physical_block_count;
         ++physical_block_index)
    {
        ...
        if ((block->flags & SPARK_KV_CACHE_BLOCK_FLAG_ALLOCATED) == 0u ||
            (block->flags & SPARK_KV_CACHE_BLOCK_FLAG_RESIDENT) == 0u ||
            SparkKvCachePhysicalBlockIsProtected(
                protected_physical_block_indices,
                protected_physical_block_count,
                physical_block_index))
        {
    ...
    uint32_t *evicted_block_count_out)
{
    ...
    while (arena->resident_block_count > max_resident_block_count)
    {
        ...
        victim_physical_block_index = SparkKvCacheSelectResidentEvictionVictim(
            arena,
            protected_physical_block_indices,
            protected_physical_block_count);
        if (victim_physical_block_index == SPARK_KV_CACHE_NO_BLOCK)
        {
            break;
        }
    ...
    const SparkKvCachePrefetchBlock *prefetch_block)
{
    ...
    if (source_entry->physical_block_index != SPARK_KV_CACHE_NO_BLOCK &&
